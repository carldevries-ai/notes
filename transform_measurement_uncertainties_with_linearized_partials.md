...
The motivation for this was that I was simulating a system with monocular visual 
...
to know evaluate the filter performance on inertial state because they cannot be
expected to converge. However, this is a generic development.

Say we have the true state $x$ vector, and the non-linear function of the states, 
$y=f(x)$. The general first order Taylor series approximation is
...
and substitute in the first order Taylor series of $y$ where that is the truth
state linearized about the estimated state. I think this makes sense since the 
filter estimate is known in the filter, but the truth is not. Let's look at a 
